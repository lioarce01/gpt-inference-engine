"""
Unified checkpoint manager for OptimizedGPT models.
...
Always uses OptimizedGPT architecture (GQA, RMSNorm, RoPE, SwiGLU).
"""
...
class CheckpointManager:
    """
    Unified checkpoint manager for loading and saving OptimizedGPT models.

    Automatically handles:
    - OptimizedGPT checkpoints (GQA, RMSNorm, RoPE, SwiGLU)
    - LoRA checkpoints (OptimizedGPT with LoRA adapters)
    ...
    def detect_checkpoint_type(ckpt_path: str) -> str:
        ...
        Returns:
            'standard': Standard OptimizedGPT checkpoint
    ...
    def load_model(
    ...
    ) -> Tuple[OptimizedGPT, Dict[str, Any]]:
        """
        Load an OptimizedGPT model from checkpoint.

        Automatically handles:
        - OptimizedGPT checkpoints → loads as OptimizedGPT
        - LoRA checkpoints → loads with LoRA structure
        - Can apply LoRA to checkpoints (for fine-tuning)

        Args:
            ckpt_path: Path to checkpoint file
            device: Device to load model on
            apply_lora: If True, apply LoRA to the checkpoint
            lora_config: LoRA configuration if apply_lora=True
                        {rank, alpha, dropout, targets}

        Returns:
            (model, metadata) tuple where model is OptimizedGPT
        """
        ...
        # Always use OptimizedGPT (simplified architecture)
        print("Loading OptimizedGPT model (RMSNorm + RoPE + GQA + SwiGLU)")
        # Filter out n_kv_head from model_args for OptimizedGPTConfig
        opt_model_args = {k: v for k, v in model_args.items() if k != 'n_kv_head'}
        opt_model_args['n_kv_head'] = model_args.get('n_kv_head', None)
        gptconf = OptimizedGPTConfig(**opt_model_args)
        model = OptimizedGPT(gptconf)
    ...
    @staticmethod
    def save_checkpoint(
        model: nn.Module,  # Accepts both GPT and OptimizedGPT
        save_dir: str,
        ...
        Args:
            model: OptimizedGPT model to save
